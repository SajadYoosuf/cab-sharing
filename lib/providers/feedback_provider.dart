...
class FeedbackProvider extends ChangeNotifier {
  ...
  Future<bool> submitFeedback(RideFeedback feedback) async {
    _isLoading = true;
    _error = null;
    notifyListeners();
  ...
  Stream<List<RideFeedback>> getRideFeedbacks(String rideId) {
    return _firestore
        .collection('feedbacks')
        .where('rideId', isEqualTo: rideId)
        .snapshots()
        .map((snapshot) => snapshot.docs
            .map((doc) => RideFeedback.fromMap(doc.id, doc.data() as Map<String, dynamic>))
            .toList());
  }

  Stream<List<RideFeedback>> getHostFeedbacks(String hostId) {
    return _firestore
        .collection('feedbacks')
  ...
  Stream<List<RideFeedback>> getAllFeedbacks() {
    return _firestore
        .collection('feedbacks')
